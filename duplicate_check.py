			...
			if entry.is_file():
				...
				if code is not None:
					if not hashmap.get(code):
						...
						if not express:
							displayPreview(hashmap[code],entry.path)
							if manual:
								ans = input("Move the duplicate, Enter Key to continue, n to not move? >>>>>")
								if not ans:
									moveItem(entry.path,entry.name)
							elif not manual:
								moveItem(entry.path,entry.name)
						else:
							moveItem(entry.path,entry.name)
...
def moveItem(start,end):
	try:
		shutil.move(start,path.join(new_path,end))
		print(f"Duplicate file found: {end}")
	except Exception as e:
		print(e)

def checkImageOrVideo(file):
	video_format = ["MKV","WEBM","MPG","MP2","MPEG","MPE","MPV","OGG","MP4","M4P","M4V","AVI","WMV","MOV","QT","FLV","SWF","AVCHD"]
...
def displayPreview(origin,duplicate):
	file_format = checkImageOrVideo(duplicate)
	if file_format == "image":
		showImage(origin,duplicate)
	elif file_format == "video":
		showVideo(origin,duplicate)
...
def showImage(origin,duplicate):
	...
	pic1,pic2 = cv2.imdecode(np.fromfile(origin, dtype=np.uint8), cv2.IMREAD_UNCHANGED),cv2.imdecode(np.fromfile(duplicate, dtype=np.uint8), cv2.IMREAD_UNCHANGED)
	try:
